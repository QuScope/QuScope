...
class EELSAnalyzer:
    ...
    def preprocess_spectrum(
        self, spectrum_data=None, energy_axis=None, use_quantum_enhancement=True
    ):
        """
        ...
        if use_quantum_enhancement:
            processed_spectrum = (
                self.preprocessor.quantum_richardson_lucy_deconvolution(spectrum_data)
            )
            ...
            for edge_type, edge_dict in self.edge_ranges.items():
                for element, (e_min, e_max) in edge_dict.items():
                    if e_min <= peak_energy <= e_max:
                        ...
                        detected_elements[element]["quantum_signature"] += peak_info[
                            "quantum_signature"
                        ]
    ...
    def quantum_enhanced_peak_detection(self, spectrum_data, energy_axis):
        ...
        peaks, properties = signal.find_peaks(
            spectrum_data, prominence=0.02 * np.max(spectrum_data), distance=5, width=1
        )
        ...
        for i, peak_idx in enumerate(peaks):
            ...
            quantum_analysis = self.quantum_peak_analysis(
                local_spectrum, local_energy, peak_energy
            )
            ...
            quantum_peaks[peak_energy] = {
                ...
                "width": (
                    properties["widths"][i] if i < len(properties["widths"]) else 0
                ),
    ...
    def quantum_peak_analysis(self, local_spectrum, local_energy, peak_energy):
        ...
        try:
            ...
            signature = self.feature_extractor.calculate_quantum_signature(
                statevector, n_peak_qubits
            )
            entanglement = self.feature_extractor.calculate_entanglement_entropy(
                statevector, n_peak_qubits
            )
    ...
    def comprehensive_analysis(
        ...
        detected_elements = self.detect_elements_from_spectrum(
        ...
        quantum_features = (
            self.feature_extractor.extract_comprehensive_quantum_features(
                processed_spectrum, energy_axis
            )
        )
        ...
        material_identification = self.generate_material_identification(
            temp_analysis_results
        )
        ...
        print(
            f"Analysis included: {len(comprehensive_results.keys())} different analysis modules"
        )
    ...
    def advanced_bonding_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        for region_name, (e_min, e_max) in energy_regions.items():
            ...
            if np.any(mask):
                ...
                bonding_analysis[region_name] = self.analyze_bonding_region(
                    region_spectrum, region_energy, detected_elements
                )

        # Quantum many-body analysis
        bonding_analysis["many_body_effects"] = self.quantum_many_body_analysis(
            spectrum_data, energy_axis, detected_elements
        )

        # Charge transfer analysis
        bonding_analysis["charge_transfer"] = self.charge_transfer_analysis(
            spectrum_data, energy_axis, detected_elements
        )

        # Hybridization analysis
        bonding_analysis["hybridization"] = self.hybridization_analysis(
            spectrum_data, energy_axis, detected_elements
        )
    ...
    def analyze_bonding_region(self, region_spectrum, region_energy, detected_elements):
        ...
        peaks, properties = signal.find_peaks(
            region_spectrum, prominence=0.1 * np.max(region_spectrum), distance=3
        )
        ...
        if len(peaks) > 0:
            ...
            if len(peaks) > 1:
                ...
                region_analysis["orbital_splitting"] = {
                    ...
                    "crystal_field_strength": self.estimate_crystal_field_strength(
                        splittings
                    ),
                }
            ...
            if len(peaks) >= 2:
                intensity_ratio = (
                    peak_intensities[0] / peak_intensities[1]
                    if peak_intensities[1] > 0
                    else float("inf")
                )
            ...
            region_analysis["quantum_interference"] = (
                self.calculate_quantum_interference_bonding(
    ...
    def calculate_quantum_interference_bonding(self, spectrum, energy, peak_energies):
        ...
        return interference_score / max(
            1, len(peak_energies) * (len(peak_energies) - 1) / 2
        )
    ...
    def quantum_many_body_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        # Plasmon analysis
        many_body_effects["plasmons"] = self.analyze_plasmons(
            spectrum_data, energy_axis, detected_elements
        )

        # Satellite peak analysis
        many_body_effects["satellite_peaks"] = self.analyze_satellite_peaks(
            spectrum_data, energy_axis
        )

        # Shake-up/shake-off analysis
        many_body_effects["shake_up_processes"] = self.analyze_shake_processes(
            spectrum_data, energy_axis
        )

        # Correlation effects
        many_body_effects["correlation_effects"] = self.analyze_correlation_effects(
            spectrum_data, energy_axis
        )
    ...
    def analyze_satellite_peaks(self, spectrum_data, energy_axis):
        ...
        main_peaks, _ = signal.find_peaks(
            spectrum_data, prominence=0.2 * np.max(spectrum_data), distance=20
        )
        ...
        for main_peak_idx in main_peaks:
            ...
            satellites, _ = signal.find_peaks(
                local_spectrum, prominence=0.05 * main_intensity, distance=5
            )
    ...
    def analyze_shake_processes(self, spectrum_data, energy_axis):
        ...
        main_peaks, _ = signal.find_peaks(
            spectrum_data, prominence=0.3 * np.max(spectrum_data)
        )
        ...
        if len(main_peaks) > 0:
            ...
            if total_intensity > 0:
                shake_analysis["shake_up_probability"] = 1 - (
                    total_peak_intensity / total_intensity
                )
    ...
    def analyze_correlation_effects(self, spectrum_data, energy_axis):
        ...
        peaks, properties = signal.find_peaks(
            spectrum_data, prominence=0.1 * np.max(spectrum_data), width=1
        )
            ...
            for i, peak_idx in enumerate(peaks):
                if i < len(widths):
                    ...
                    if len(left_half) > 0 and len(right_half) > 0:
                        asymmetry = (np.mean(right_half) - np.mean(left_half)) / (
                            np.mean(right_half) + np.mean(left_half)
                        )
                        correlation_analysis["final_state_relaxation"][
                            f"peak_{i}"
                        ] = asymmetry
    ...
    def charge_transfer_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        for element in detected_elements:
            if element in self.edge_ranges["K_edges"]:
                ...
                mask = (energy_axis >= k_edge_range[0] - 10) & (
                    energy_axis <= k_edge_range[1] + 10
                )
                if np.any(mask):
    ...
    def hybridization_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        if "C" in detected_elements:
            hybrid_analysis["carbon_hybridization"] = (
                self.analyze_carbon_hybridization_detailed(spectrum_data, energy_axis)
            )
        ...
        for element in detected_elements:
            if element in tm_elements:
                hybrid_analysis["transition_metal_hybridization"][element] = (
                    self.analyze_tm_hybridization_detailed(
    ...
    def analyze_carbon_hybridization_detailed(self, spectrum_data, energy_axis):
        ...
        total_intensity = (
            carbon_analysis["pi_star_intensity"]
            + carbon_analysis["sigma_star_intensity"]
        )
        if total_intensity > 0:
            # Strong π* indicates sp2
            carbon_analysis["sp2_fraction"] = (
                carbon_analysis["pi_star_intensity"] / total_intensity
            )
    ...
    def analyze_tm_hybridization_detailed(self, spectrum_data, energy_axis, element):
        ...
        if (
            element in self.edge_ranges["L2_edges"]
            and element in self.edge_ranges["L3_edges"]
        ):
            l3_range = self.edge_ranges["L3_edges"][element]
            l2_range = self.edge_ranges["L2_edges"][element]

            # Analyze L3 edge fine structure
            l3_mask = (energy_axis >= l3_range[0] - 5) & (
                energy_axis <= l3_range[1] + 15
            )
            if np.any(l3_mask):
                ...
                peaks, _ = signal.find_peaks(
                    l3_spectrum, prominence=0.1 * np.max(l3_spectrum)
                )
    ...
    def advanced_magnetic_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        for element in magnetic_elements:
            if (
                ...
                mag_analysis = self.analyze_magnetic_dichroism(
                    spectrum_data, energy_axis, element
                )
                magnetic_analysis["magnetic_moments"][element] = mag_analysis

        # Exchange interaction analysis
        magnetic_analysis["exchange_interactions"] = self.analyze_exchange_interactions(
            spectrum_data, energy_axis, magnetic_elements
        )
        # Spin orbit coupling analysis
        magnetic_analysis["spin_orbit_coupling"] = self.analyze_spin_orbit_coupling(
            spectrum_data, energy_axis, magnetic_elements
        )
        # Estimate magnetic ordering temperature
        if magnetic_elements:
            magnetic_analysis["curie_temperature_estimate"] = (
                self.estimate_curie_temperature(
                    magnetic_elements, magnetic_analysis["exchange_interactions"]
                )
            )
    ...
    def analyze_magnetic_dichroism(self, spectrum_data, energy_axis, element):
        ...
        if l2_range and l3_range:
            # Extract L2 range
            l2_mask = (energy_axis >= l2_range[0] - 5) & (
                energy_axis <= l2_range[1] + 10
            )
            l2_spectrum = spectrum_data[l2_mask] if np.any(l2_mask) else np.array([])

            # Extract L3 range
            l3_mask = (energy_axis >= l3_range[0] - 5) & (
                energy_axis <= l3_range[1] + 10
            )
    ...
    def analyze_exchange_interactions(
        self, spectrum_data, energy_axis, magnetic_elements
    ):
        """
            ...
            if element in self.edge_ranges["L3_edges"]:
                ...
                if np.any(mask):
                    ...
                    if len(peaks) >= 2:
                        ...
                        exchange_analysis["exchange_constants"][element] = (
                            exchange_splitting / 2
                        )

        return exchange_analysis

    def analyze_spin_orbit_coupling(
        self, spectrum_data, energy_axis, magnetic_elements
    ):
        """
        ...
        for element in magnetic_elements:
            if (
                ...
                if element in self.magnetic_moments:
                    ...
                    soc_analysis["orbital_angular_momentum"][
                        element
    ...
    def energy_trajectory_analysis(self, spectrum_data, energy_axis):
        ...
        trajectory_analysis["energy_loss_function"] = (
            self.calculate_energy_loss_function(spectrum_data, energy_axis)
        )

        # Analyze scattering processes
        trajectory_analysis["scattering_cross_sections"] = (
            self.analyze_scattering_processes(spectrum_data, energy_axis)
        )

        # Quantum trajectory analysis
        trajectory_analysis["quantum_trajectories"] = self.quantum_trajectory_analysis(
            spectrum_data, energy_axis
        )
    ...
    def kramers_kronig_analysis(self, spectrum_data, energy_axis):
        ...
        quantum_kk_results = self.preprocessor.quantum_kramers_kronig_transform(
            energy_axis, spectrum_data
        )
    ...
    def analyze_scattering_processes(self, spectrum_data, energy_axis):
        ...
        if np.any(zero_loss_mask):
            ...
            if total_intensity > 0:
                scattering_analysis["elastic_scattering"] = (
                    elastic_intensity / total_intensity
                )
                scattering_analysis["inelastic_scattering"] = (
                    1 - scattering_analysis["elastic_scattering"]
                )
        ...
        if np.any(high_energy_mask):
            ...
            if total_intensity > 0:
                scattering_analysis["multiple_scattering"] = (
                    high_energy_intensity / total_intensity
                )
    ...
    def quantum_trajectory_analysis(self, spectrum_data, energy_axis):
        ...
        try:
            ...
            if n_traj_qubits >= 2:
                quantum_traj["trajectory_entanglement"] = (
                    self.feature_extractor.calculate_entanglement_entropy(
    ...
    def phonon_magnon_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        if np.any(low_energy_mask):
            excitation_analysis["phonons"] = self.analyze_phonon_modes(
                spectrum_data[low_energy_mask],
        ...
        if any(elem in detected_elements for elem in magnetic_elements):
            excitation_analysis["magnons"] = self.analyze_magnon_modes(
                spectrum_data, energy_axis, detected_elements
            )

        # Electron-phonon coupling
        excitation_analysis["electron_phonon_coupling"] = (
            self.analyze_electron_phonon_coupling(
                spectrum_data, energy_axis, detected_elements
            )
        )
    ...
    def analyze_phonon_modes(
        self, low_energy_spectrum, low_energy_axis, detected_elements
    ):
        """
                ...
                if element in self.phonon_energies:
                    ...
                    for mode_name, mode_energy in element_phonons.items():
                        if abs(peak_energy - mode_energy) < 5:  # 5 meV tolerance
                            phonon_analysis["mode_assignment"][
                                f"{element}_{mode_name}"
                            ] = {
                                "energy_meV": peak_energy,
    ...
    def analyze_magnon_modes(self, spectrum_data, energy_axis, detected_elements):
        ...
        if np.any(magnon_mask):
            ...
            peaks, _ = signal.find_peaks(
                magnon_spectrum, prominence=0.02 * np.max(magnon_spectrum), distance=5
            )
        ...
        magnetic_elements = [
            ...
            for elem in detected_elements
            if elem
            in ["Fe", "Co", "Ni", "Mn", "Cr", "Gd", "Dy", "Ho", "Tb", "Tm", "Er"]
        ]
    ...
    def analyze_electron_phonon_coupling(
        self, spectrum_data, energy_axis, detected_elements
    ):
        """
        ...
        main_peaks, properties = signal.find_peaks(
            spectrum_data, prominence=0.2 * np.max(spectrum_data), distance=10
        )
        ...
        if len(main_peaks) > 0 and "widths" in properties:
            ...
            if avg_width > lifetime_width * 2:
                coupling_analysis["coupling_strength"] = min(
                    (avg_width / lifetime_width - 1), 5.0
                )
    ...
    def vibrational_eels_analysis(self, spectrum_data, energy_axis, detected_elements):
        ...
        if np.any(vib_mask):
            ...
            vibrational_analysis["molecular_vibrations"] = (
                self.analyze_molecular_vibrations(
                    vib_spectrum, vib_energy, detected_elements
                )
            )
            vibrational_analysis["surface_modes"] = self.analyze_surface_vibrations(
                vib_spectrum, vib_energy, detected_elements
            )
            vibrational_analysis["defect_modes"] = self.analyze_defect_vibrations(
                vib_spectrum, vib_energy
            )

        return vibrational_analysis

    def analyze_molecular_vibrations(
        self, vib_spectrum, vib_energy_meV, detected_elements
    ):
        """
        ...
        peaks, _ = signal.find_peaks(
            vib_spectrum, prominence=0.03 * np.max(vib_spectrum), distance=2
        )
    ...
    def analyze_surface_vibrations(
        self, vib_spectrum, vib_energy_meV, detected_elements
    ):
        """
        ...
        surface_peaks, _ = signal.find_peaks(
            vib_spectrum, prominence=0.05 * np.max(vib_spectrum), distance=3
        )
        for peak_idx in surface_peaks:
            ...
            if peak_energy < 50:  # meV
                surface_analysis["surface_phonons"].append(
                    {"energy_meV": peak_energy, "type": "surface_phonon"}
                )
    ...
    def analyze_defect_vibrations(self, vib_spectrum, vib_energy_meV):
        ...
        """
        defect_analysis = {"local_modes": [], "gap_modes": [], "disorder_effects": 0.0}

        # Defect modes usually appear as weak, isolated peaks
        defect_peaks, properties = signal.find_peaks(
            vib_spectrum,
        ...
        if len(defect_peaks) > 0:
            avg_width = np.mean(
                [
                    properties["widths"][i] if i < len(properties["widths"]) else 1
    ...
    def perform_quantum_ml_analysis(self, quantum_features):
        ...
        try:
            ...
            if len(feature_vector) > 0:
                qml_prediction = self.qml_processor.quantum_kernel_prediction(
                    [feature_vector]
                )
    ...
    def predict_comprehensive_material_properties(
        self, detected_elements, bonding_analysis, magnetic_analysis, kk_results
    ):
        """
        ...
        """
        properties = {
            "mechanical_properties": self.predict_mechanical_properties(
                detected_elements, bonding_analysis
            ),
            "electrical_properties": self.predict_electrical_properties(
                detected_elements, bonding_analysis
            ),
            "thermal_properties": self.predict_thermal_properties(
                detected_elements, bonding_analysis
            ),
            ...
            "chemical_properties": self.predict_chemical_properties(
                detected_elements, bonding_analysis
            ),
        }
    ...
    def predict_mechanical_properties(self, detected_elements, bonding_analysis):
        ...
        primary_bonding = bonding_analysis.get("bonding_characteristics", {}).get(
            "primary_bonding", "Unknown"
        )
    ...
    def extract_optical_properties(self, kk_results):
        ...
        if isinstance(kk_results, np.ndarray) and len(kk_results) > 0:
            optical["refractive_index_range"] = (
                f"{np.min(kk_results):.2f} - {np.max(kk_results):.2f}"
            )
    ...
    def extract_magnetic_predictions(self, magnetic_analysis):
        ...
        if magnetic_analysis.get("magnetic_elements"):
            ...
            predictions["magnetic_ordering"] = magnetic_analysis.get(
                "magnetic_ordering", "Unknown"
            )
    ...
    def calculate_comprehensive_confidence(
        self, detected_elements, quantum_features, bonding_analysis
    ):
        """
        ...
        if detected_elements:
            element_confidences = [
                info.get("confidence", 0) for info in detected_elements.values()
            ]
        ...
        bonding_confidence = bonding_analysis.get("bonding_characteristics", {}).get(
            "confidence", 0
        )
    ...
    def visualize_results(
        ...
        if show_quantum_features and analysis_results.get("quantum_features"):
            ...
            if feature_names:
                bars = ax4.bar(
                    range(len(feature_names)), feature_values, color="cyan", alpha=0.7
                )
    ...
    def generate_comprehensive_report(
        self, analysis_results, include_quantum_details=True
    ):
        """
        ...
        if include_quantum_details:
            report["quantum_insights"] = self.generate_quantum_insights(
                analysis_results
            )
    ...
    def generate_executive_summary(self, analysis_results):
        ...
        bonding_info = analysis_results.get("bonding_analysis", {}).get(
            "bonding_characteristics", {}
        )
        if bonding_info.get("primary_bonding") != "Unknown":
            summary["primary_characteristics"].append(
                f"Primary bonding: {bonding_info['primary_bonding']}"
            )

        material_id = analysis_results.get("material_identification", {})
        if material_id.get("material_class") != "Unknown":
            summary["primary_characteristics"].append(
                f"Material class {material_id['material_class']}"
            )
        ...
        electrical = material_props.get("electrical_properties", {})
        if electrical.get("conductivity_type") != "Unknown":
            summary["key_properties"].append(
                f"Electrical: {electrical['conductivity_type']}"
            )

        mechanical = material_props.get("mechanical_properties", {})
        if mechanical.get("hardness_estimate", "Unknown") != "Unknown":
            summary["key_properties"].append(
                f"Hardness: {mechanical['hardness_estimate']}"
            )
        ...
        if (
            material_id.get("specific_identification", "Unidentified")
            != "unidentified_ceramic"
        ):
            specific_name = (
                material_id.get("specific_identification", "").replace("_", " ").title()
            )
    ...
    def generate_detailed_analysis(self, analysis_results):
        ...
        detailed = {
            ...
            "spectroscopic_features": self.format_spectroscopic_features(
                analysis_results
            ),
    ...
    def format_spectroscopic_features(self, analysis_results):
        ...
        features = {
            ...
            "quantum_fourier_analysis": (
                "Applied" if analysis_results.get("qft_results", [{}])[0] else "Failed"
            ),
            "kramers_kronig_analysis": (
                "Applied"
                if analysis_results.get("optical_properties")
                else "Not available"
            ),
        }
    ...
    def format_bonding_analysis(self, bonding_analysis):
        ...
        """
        bonding_report = {
            "primary_bonding_type": bonding_analysis.get(
                "bonding_characteristics", {}
            ).get("primary_bonding", "Unknown"),
            "bonding_confidence": bonding_analysis.get(
                "bonding_characteristics", {}
            ).get("confidence", 0),
    ...
    def format_alternative_classifications(alternatives):
        ...
        for alt in alternatives[:2]:  # Show top 2 alternatives
            ...
            formatted_alts.append(
                f"{material_class}/{material_subclass} (confidence: {confidence:.2f})"
            )
    ...
    def generate_material_identification(self, analysis_results):
        ...
        identification["stoichiometry_analysis"] = self.analyze_stoichiometry(
            detected_elements
        )
        ...
        polymer_match = self.classify_polymers(
            element_list, detected_elements, bonding_analysis
        )
        if polymer_match["confidence"] > 0.3:
            classification_results.append(polymer_match)

        # 2. Check for metals and alloys
        metal_match = self.classify_metals_alloys(
            element_list, detected_elements, bonding_analysis
        )
        ...
        semiconductor_match = self.classify_semiconductors(
            element_list, detected_elements, optical_properties
        )
        if semiconductor_match["confidence"] > 0.3:
            classification_results.append(semiconductor_match)

        # 4. Check for ceramics
        ceramic_match = self.classify_ceramics(
            element_list, detected_elements, bonding_analysis
        )
        ...
        # 5. Check for glasses
        glass_match = self.classify_glasses(
            element_list, detected_elements, bonding_analysis
        )
        ...
        composite_match = self.classify_composites(
            element_list, detected_elements, bonding_analysis
        )
        ...
        nano_match = self.classify_nanomaterials(
            element_list, detected_elements, quantum_features
        )
        if nano_match["confidence"] > 0.3:
            classification_results.append(nano_match)

        # 8. Check for biomaterials
        bio_match = self.classify_biomaterials(
            element_list, detected_elements, bonding_analysis
        )
        ...
        if classification_results:
            ...
            identification = self.fallback_classification(
                element_list, detected_elements, bonding_analysis
            )

        # Predict structure and properties
        identification["structure_prediction"] = self.predict_structure(
            element_list, identification
        )
    ...
    def analyze_stoichiometry(self, detected_elements):
        ...
        total_intensity = sum(
            info.get("intensity", 0) for info in detected_elements.values()
        )
        ...
        if stoichiometry["atomic_ratios"]:
            max_element = max(
                stoichiometry["atomic_ratios"], key=stoichiometry["atomic_ratios"].get
            )
            ...
            stoichiometry["trace_elements"] = [
                elem
                for elem, ratio in stoichiometry["atomic_ratios"].items()
                if ratio < threshold
            ]
    ...
    def classify_polymers(self, element_list, detected_elements, bonding_analysis):
        ...
        if has_carbon:
            organic_fraction = len(set(element_list) & organic_elements) / len(
                element_list
            )
            ...
            if primary_bonding == "Covalent":
                base_confidence += 0.2
                classification["classification_reasoning"].append(
                    "Covalent bonding supports polymer structure"
                )
            ...
            if organic_fraction > 0.8:
                ...
                if set(element_list) == {"C", "H"}:
                    ...
                    classification["specific_identification"] = (
                        "Polyethylene or Polypropylene"
                    )
                    classification["confidence"] = 0.8
                    classification["classification_reasoning"].append(
                        "Pure hydrocarbon composition"
                    )

                elif "O" in element_list and len(element_list) <= 3:
                    co_ratio = detected_elements.get("C", {}).get("intensity", 0) / max(
                        detected_elements.get("O", {}).get("intensity", 1), 1
                    )
                    if 2 < co_ratio < 10:
                        classification["material_subclass"] = (
                        ...
                        classification["classification_reasoning"].append(
                            f"C/O ratio ({co_ratio:.1f}) suggests polyester/polyether"
                        )
                    ...
                    if "O" in element_list:
                        ...
                        classification["specific_identification"] = (
                            "Polyimide or Polyaniline"
                        )
                    classification["confidence"] = 0.7
                    classification["classification_reasoning"].append(
                        "Nitrogen content suggests specialty polymer"
                    )
                    ...
                    classification["classification_reasoning"].append(
                        "Chlorine suggests PVC or similar"
                    )

                elif "F" in element_list:
                    classification["material_subclass"] = "Fluoropolymer"
                    classification["specific_identification"] = (
                        "PTFE or Fluorinated_Polymer"
                    )
                    classification["confidence"] = 0.9
                    classification["classification_reasoning"].append(
                        "Fluorine indicates fluoropolymer"
                    )
        ...
        # Check for inorganic polymers
            ...
            if set(element_list).issubset(si_o_elements):
                ...
                classification["confidence"] = 0.8
                classification["classification_reasoning"].append(
                    "Si-O backbone indicates silicone"
                )
    ...
    def classify_metals_alloys(self, element_list, detected_elements, bonding_analysis):
        ...
        metallic_fraction = (
            len(metallic_elements) / len(element_list) if element_list else 0
        )
        ...
        if primary_bonding == "Metallic":
            base_confidence = 0.9
            classification["classification_reasoning"].append(
                "Metallic bonding confirmed"
            )
        elif metallic_fraction > 0.7:
            base_confidence = 0.7
            classification["classification_reasoning"].append(
                "High metallic element content"
            )
        else:
            base_confidence = 0.3
        ...
        if metallic_fraction > 0.5:
            ...
            if len(element_list) == 1 and element_list[0] in all_metals:
                ...
                classification["classification_reasoning"].append(
                    f"Single metallic element: {metal}"
                )
            ...
            # Alloy identification
                ...
                classification = self.identify_specific_alloy(
                    classification, element_list, detected_elements
                )
            ...
            # Metal with non-metallic additions (could be alloy or compound)
                ...
                if non_metals <= {
                    ...
                    classification["material_subclass"] = "Alloy"
                    classification["specific_identification"] = (
                        f"{metal.lower()}-Based Alloy"
                    )
                    classification["confidence"] = 0.7
                    classification["classification_reasoning"].append(
                        f"{metal} with common alloying elements"
                    )
                else:
                    classification[
                        "confidence"
                    ] *= 0.7  # Reduce confidence for unusual composition
    ...
    def classify_semiconductors(
        self, element_list, detected_elements, optical_properties
    ):
        """
        ...
        if len(element_list) == 1 and element_list[0] in group_iv:
            ...
            classification["specific_identification"] = (
                f"{element.lower()} Semiconductor"
            )
            classification["confidence"] = 0.9
            classification["classification_reasoning"].append(
                f"Pure Group IV semiconductor: {element}"
            )
        ...
        # III-V semiconductors
            ...
            if len(iii_elements) == 1 and len(v_elements) == 1:
                ...
                classification["specific_identification"] = (
                    f"{iii_elem.lower()}{v_elem.lower()} Semiconductor"
                )
                classification["confidence"] = 0.85
                classification["classification_reasoning"].append(
                    f"III-V compound: {iii_elem}{v_elem}"
                )
            ...
            # II-VI semiconductors
                ...
                if len(ii_elements) == 1 and len(vi_elements) == 1:
                    ...
                    classification["specific_identification"] = (
                        f"{ii_elem.lower()}{vi_elem.lower()} Semiconductor"
                    )
                    classification["confidence"] = 0.8
                    classification["classification_reasoning"].append(
                        f"II-VI compound: {ii_elem}{vi_elem}"
                    )
        ...
        # Organic semiconductors
            ...
            if "C" in element_list:
                ...
                classification["classification_reasoning"].append(
                    "Organic composition suggests organic semiconductor"
                )
    ...
    def classify_ceramics(self, element_list, detected_elements, bonding_analysis):
        ...
        if primary_bonding == "Ionic":
            base_confidence = 0.8
            classification["classification_reasoning"].append(
                "Ionic bonding characteristic of ceramics"
            )
        else:
            base_confidence = 0.5
            ...
            if metals & ceramic_metals:
                ...
                # Complex oxides
                    ...
                    if element_set == {"Ba", "Ti", "O"}:
                        ...
                        classification["functional_properties"] = [
                            "Ferroelectric",
                            "Piezoelectric",
                        ]
                        classification["confidence"] = 0.9
    ...
    def classify_glasses(self, element_list, detected_elements, bonding_analysis):
        ...
        if "Si" in element_list and "O" in element_list:
            ...
            if modifiers & glass_modifiers:
                ...
                classification["classification_reasoning"].append(
                    "Si-O network with glass modifiers"
                )
        ...
        # Metallic glasses
            ...
            {"O", "C", "N", "H"} & element_set
        ):  # No obvious non-metallic glass formers
            metals = {
                "Fe",
                "Co",
                "Ni",
            ...
            if metal_elements and metalloid_elements:
                ...
                classification["confidence"] = 0.6
                classification["classification_reasoning"].append(
                    "Metal + metalloid composition suggests metallic glass"
                )
        ...
        # Chalcogenide glasses
            ...
            if network_formers:
                ...
                classification["confidence"] = 0.75
                classification["classification_reasoning"].append(
                    "Chalcogen + network former composition"
                )

                if "As" in element_list and "S" in element_list:
                    classification["specific_identification"] = "Arsenic Sulfide Glass"
                    classification["confidence"] = 0.8
                elif "Ge" in element_list and (
                    "S" in element_list or "Se" in element_list
                ):
                    classification["specific_identification"] = (
                        "Germanium Chalcogenide Glass"
                    )
                    classification["confidence"] = 0.8
        ...
        # Phosphate glasses
            ...
            classification["confidence"] = 0.7
            classification["classification_reasoning"].append(
                "P-O network indicates phosphate glass"
            )
        ...
        # Borate glasses
            ...
            classification["confidence"] = 0.7
            classification["classification_reasoning"].append(
                "B-O network indicates borate glass"
            )
    ...
    def classify_composites(self, element_list, detected_elements, bonding_analysis):
        ...
        if len(element_list) >= 3:
            ...
            if organic_elements and reinforcement_elements:
                ...
                classification["confidence"] = 0.6
                classification["classification_reasoning"].append(
                    "Organic matrix with inorganic reinforcement"
                )
            ...
            if metal_elements and reinforcement_elements and len(metal_elements) >= 1:
                ...
                classification["classification_reasoning"].append(
                    "Metal matrix with ceramic/carbon reinforcement"
                )

                primary_metal = max(
                    metal_elements,
                    key=lambda x: detected_elements.get(x, {}).get("intensity", 0),
                )
                classification["matrix_material"] = f"{primary_metal.lower()} Matrix"

                if "C" in reinforcement_elements:
                    classification["specific_identification"] = (
                        f"{primary_metal.lower()} Carbon Composite"
                    )
                elif "Si" in reinforcement_elements and "C" in reinforcement_elements:
                    classification["specific_identification"] = (
                        f"{primary_metal.lower()} Si-C Composite"
                    )
            ...
            if len(element_set & ceramic_elements) >= 3:
                ...
                classification["confidence"] = 0.6
                classification["classification_reasoning"].append(
                    "Multi-component ceramic system"
                )
    ...
    def classify_nanomaterials(self, element_list, detected_elements, quantum_features):
        ...
        quantum_signature = 0
        if quantum_features:
            entanglement = quantum_features.get("entanglement_features", {}).get(
                "multipartite_entanglement", 0
            )
            coherence = quantum_features.get("coherence_features", {}).get(
                "l1_coherence", 0
            )
            quantum_signature = (entanglement + coherence) / 2

        base_confidence = (
            0.3 + quantum_signature * 0.4
        )  # Quantum effects suggest nanoscale
        ...
        if element_set == {"C"} or element_set == {"C", "H"}:
            ...
            classification["classification_reasoning"].append(
                "Pure carbon suggests carbon nanomaterial"
            )
            ...
            if quantum_signature > 0.5:
                ...
                classification["specific_identification"] = (
                    "Carbon Nanotubes or Carbon Nanosheet"
                )
                classification["confidence"] = 0.7
            else:
        ...
        # Metal nanoparticles
        elif len(element_list) == 1 and element_list[0] in {
            "Au",
            "Ag",
            "Cu",
            "Pt",
            ...
            classification["classification_reasoning"].append(
                f"Pure {metal} with quantum signatures suggests nanoparticles"
            )
        ...
        # Quantum dots
            ...
            if element_pair in semiconductor_pairs or any(
                element_pair == pair for pair in semiconductor_pairs
            ):
                classification["material_subclass"] = "Quantum Dots"
                classification["specific_identification"] = (
                    f"{element_pair[0].lower()}_{element_pair[1].lower()} Quantum Dots"
                )
                classification["confidence"] = base_confidence + 0.5
                classification["classification_reasoning"].append(
                    "Semiconductor pair with quantum effects suggests quantum dots"
                )
        ...
        # Oxide nanoparticles
            ...
            if metal in nano_oxides:
                ...
                classification["specific_identification"] = (
                    f"{metal.lower()} oxide nanoparticle"
                )
                classification["confidence"] = base_confidence + 0.3
                classification["classification_reasoning"].append(
                    f"{metal} oxide with possible nanoscale effects"
                )
    ...
    def classify_biomaterials(self, element_list, detected_elements, bonding_analysis):
        ...
        # Biominerals
            ...
            if element_set == {"Ca", "P", "O"} or element_set == {"Ca", "P", "O", "H"}:
                ...
                classification["specific_identification"] = (
                    "Hydroxyapatite or Bone Mineral"
                )
                classification["confidence"] = 0.85
                classification["classification_reasoning"].append(
                    "Ca-P-O composition characteristics of bone/teeth"
                )
                ...
                classification["confidence"] = 0.8
                classification["classification_reasoning"].append(
                    "CaCO3 composition typical of shells/coral"
                )
        ...
        # Siliceous biomaterials
            ...
            classification["confidence"] = 0.7
            classification["classification_reasoning"].append(
                "Silica composition suggests diatoms or sponges"
            )

        return classification

    def fallback_classification(
        self, element_list, detected_elements, bonding_analysis
    ):
        """
        ...
        if n_elements == 1:
            ...
            classification["specific_identification"] = (
                f"{element_list[0].lower()}_{element_list[1].lower()} compound"
            )
            ...
            classification["classification_reasoning"] = [
                f"{n_elements}-element compound"
            ]

        else:
            classification["material_class"] = "Complex Material"
            classification["specific_identification"] = (
                "Complex multi-component material"
            )
            classification["confidence"] = 0.3
            classification["classification_reasoning"] = [
                f"Complex material with {n_elements} elements"
            ]
        ...
        if primary_bonding != "Unknown":
            classification["classification_reasoning"].append(
                f"Primary bonding: {primary_bonding}"
            )
    ...
    def predict_structure(self, element_list, identification):
        ...
        if material_class == "Metal":
            ...
            if (
                "diamond" in specific_id
                or "silicon" in specific_id
                or "germanium" in specific_id
            ):
                structure_prediction["crystal_system"] = "Cubic"
            ...
            if "quantum_dots" in specific_id:
                ...
                "carbon nanosheet" in specific_id.lower()
                or "carbon nanotube" in specific_id.lower()
            ):
    ...
    def generate_recommendations(self, analysis_results):
        ...
        recommendations = {
            "analysis_quality": (
                ...
                else "good" if overall_confidence > 0.6 else "needs_improvement"
            ),
        ...
        if confidence.get("element_detection", 0) < 0.7:
            recommendations["suggested_improvements"].append(
                "Improve energy resolution for better element detection"
            )

        if confidence.get("quantum_analysis", 0) < 0.7:
            recommendations["suggested_improvements"].append(
                "Increase quantum circuit depth for better feature extraction"
            )

        detected_elements = analysis_results.get("detected_elements", {})
        if len(detected_elements) > 3:
            recommendations["future_analysis"].append(
                "Spatial mapping for phase distribution analysis"
            )

        magnetic_elements = analysis_results.get("magnetic_properties", {}).get(
            "magnetic_elements", {}
        )
        if magnetic_elements:
            recommendations["future_analysis"].append(
                "Magnetic dichroism measurements for spin analysis"
            )

...
class ImageDenoiser:
    ...
    def calculate_spatial_correlation(self, sorted_states, size, original_patch):
        ...
        for bitstring, count in sorted_states[:3]:  # Top 3 states
            ...
            if len(bitstring_clean) == size * size:
                ...
                correlation = np.corrcoef(pattern.flatten(), original_patch.flatten())[
                    0, 1
                ]
        ...
        for i in range(0, h, patch_h):
            for j in range(0, w, patch_w):
                if patch_idx < len(all_quantum_features):
                    ...
                    correlation_map[i:end_i, j:end_j] = features.get(
    ...
    def adaptive_classical_denoising(self, img_array, feature_maps):
        ...
        denoised[med_conf_mask] = (
            0.7 * denoised_median[med_conf_mask] + 0.3 * img_array[med_conf_mask]
        )
    ...
    def process_image(self, image_path):
        ...
        feature_maps = self.decode_quantum_features(
            all_quantum_features, original_shape
        )
    ...
    def calculate_metrics(self, results):
        ...
        original_edges = np.sqrt(
            sobel(original, axis=0) ** 2 + sobel(original, axis=1) ** 2
        )
        denoised_edges = np.sqrt(
            sobel(denoised, axis=0) ** 2 + sobel(denoised, axis=1) ** 2
        )
        edge_preservation = np.corrcoef(
            original_edges.flatten(), denoised_edges.flatten()
        )[0, 1]
        ...
        print(
            f"Mean Quantum Confidence: {np.mean(results['feature_maps']['confidence']):.3f}"
        )
        print(
            f"Mean Quantum Entropy: {np.mean(results['feature_maps']['entropy']):.3f}"
        )

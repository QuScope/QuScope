...
def validate_image_array(
    img_array: np.ndarray, allow_multichannel: bool = True
    ...
    if np.min(img_array) < 0 or np.max(img_array) > 1:
        logger.warning("Image values outside [0, 1] range. Normalizing to [0, 1].")
        # Normalize to [0, 1]
...
def calculate_required_qubits(data_size: int, encoding: EncodingMethod) -> int:
...
def encode_image_to_circuit(
    ...
    try:
        img_array, is_multichannel = validate_image_array(
            img_array, allow_multichannel=(method != EncodingMethod.BASIS)
        )
    ...
    try:
        if method == EncodingMethod.AMPLITUDE:
            ...
            return _basis_encode_image(img_array, add_measurements)
        elif method == EncodingMethod.ANGLE:
            return _angle_encode_image(img_array, num_qubits, add_measurements)
        elif method == EncodingMethod.FLEXIBLE:
            return _flexible_encode_image(img_array, num_qubits, add_measurements)
        elif method == EncodingMethod.FRQI:
            if is_multichannel:
                raise InvalidImageError(
                    "FRQI encoding currently only supports grayscale images. Convert to grayscale first."
                )
            return _frqi_encode_image(img_array, add_measurements)
        else:
...
def _amplitude_encode_image(
    ...
    if num_qubits is None:
        num_qubits = calculate_required_qubits(len(img_norm), EncodingMethod.AMPLITUDE)
...
def _basis_encode_image(
    img_array: np.ndarray, add_measurements: bool = False, threshold: float = 0.5
) -> QuantumCircuit:
...
def _frqi_encode_image(
    img_array: np.ndarray, add_measurements: bool = False
) -> QuantumCircuit:
...
def encode_binary_image(
    binary_img: np.ndarray, add_measurements: bool = False
) -> QuantumCircuit:
    """Encode a binary image into a quantum circuit using basis states.
...
def analyze_encoding_resources(
    img_array: np.ndarray, methods: List[EncodingMethod] = None
    ...
    for method in methods:
        try:
            ...
            results[method.value] = {
                ...
                "classical_bits": (
                    n_qubits if method != EncodingMethod.FRQI else n_qubits + 1
                ),
            }
        except Exception as e:
...
def encode_multichannel_image(
    ...
    if img_array.ndim != 3:
        raise InvalidImageError("Input must be a multi-channel image (3D array)")

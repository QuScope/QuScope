...
class ClassicalTEMSimulator:
    ...
    def simulate_with_coherence(
        ...
        if temporal:
            ctf = ctf.add_temporal_envelope(
                focal_spread=self.params.defocus_spread
            )
...
class ValidationMetrics:
    ...
    def calculate_ssim(
        ...
        """
        # Normalize images
        img_q = (quantum_image - quantum_image.min()) / (quantum_image.max() - quantum_image.min())
        img_c = (classical_image - classical_image.min()) / (classical_image.max() - classical_image.min())
    ...
    def calculate_all_metrics(
        ...
        """
        return {
            'fidelity': ValidationMetrics.calculate_fidelity(quantum_image, classical_image),
            ...
            'pearson': ValidationMetrics.calculate_pearson_correlation(quantum_image, classical_image),
        }
...
class ValidationVisualizer:
    ...
    def plot_comparison(
        ...
        axes[0, 0].set_title('Quantum Circuit Simulation', fontsize=12, fontweight='bold')
        axes[0, 0].axis('off')
        plt.colorbar(im1, ax=axes[0, 0], fraction=0.046)
        
        # Classical image
        im2 = axes[0, 1].imshow(classical_image, cmap='gray')
        axes[0, 1].set_title('Classical Multislice (abTEM)', fontsize=12, fontweight='bold')
        axes[0, 1].axis('off')
        ...
        axes[1, 1].plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=2, label='Ideal')
        axes[1, 1].legend()
    ...
    def plot_multi_voltage_validation(
        ...
        # Fidelity
        axes[0, 0].plot(np.array(voltages) / 1e3, fidelities, 'o-', linewidth=2, markersize=8)
        axes[0, 0].set_xlabel('Acceleration Voltage (kV)', fontsize=10)
        axes[0, 0].set_ylabel('Fidelity', fontsize=10)
        axes[0, 0].set_title('Quantum Fidelity vs Voltage', fontsize=12, fontweight='bold')
        axes[0, 0].grid(True, alpha=0.3)
        axes[0, 0].set_ylim([0.995, 1.001])
        
        # RMSE
        axes[0, 1].plot(np.array(voltages) / 1e3, rmses, 'o-', linewidth=2, markersize=8, color='orange')
        axes[0, 1].set_xlabel('Acceleration Voltage (kV)', fontsize=10)
        axes[0, 1].set_ylabel('RMSE', fontsize=10)
        axes[0, 1].set_title('Root Mean Square Error vs Voltage', fontsize=12, fontweight='bold')
        axes[0, 1].grid(True, alpha=0.3)
        
        # SSIM
        axes[1, 0].plot(np.array(voltages) / 1e3, ssims, 'o-', linewidth=2, markersize=8, color='green')
        axes[1, 0].set_xlabel('Acceleration Voltage (kV)', fontsize=10)
        axes[1, 0].set_ylabel('SSIM', fontsize=10)
        axes[1, 0].set_title('Structural Similarity vs Voltage', fontsize=12, fontweight='bold')
        axes[1, 0].grid(True, alpha=0.3)
        
        # Pearson
        axes[1, 1].plot(np.array(voltages) / 1e3, pearsons, 'o-', linewidth=2, markersize=8, color='red')
        axes[1, 1].set_xlabel('Acceleration Voltage (kV)', fontsize=10)
        axes[1, 1].set_ylabel('Pearson Correlation', fontsize=10)
        axes[1, 1].set_title('Pearson Correlation vs Voltage', fontsize=12, fontweight='bold')

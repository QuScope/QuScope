...
from classical_validation import ClassicalTEMSimulator, ValidationParameters, ValidationMetrics
...
def test_voltage_dependence(
    ...
    for voltage in voltages:
        ...
        # Get potential
        V_pot = abtem.Potential(atoms, sampling=pixel_size, gpts=(grid_size, grid_size),
                               projection='infinite')
        ...
        I_q_norm = (I_quantum - I_quantum.min()) / (I_quantum.max() - I_quantum.min() + 1e-10)
        I_c_norm = (I_classical - I_classical.min()) / (I_classical.max() - I_classical.min() + 1e-10)
        ...
        # Contrast
        contrast_q = (I_quantum.max() - I_quantum.min()) / (I_quantum.max() + I_quantum.min())
        contrast_c = (I_classical.max() - I_classical.min()) / (I_classical.max() + I_classical.min())
...
def create_summary_figure(df: pd.DataFrame, output_file: str = 'validation_summary.png'):
    ...
    for mat, color in zip(materials, colors):
        ...
        ax1.plot(df_mat['Voltage_kV'], df_mat['Wavelength_A'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax2.plot(df_mat['Voltage_kV'], df_mat['Sigma'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax3.plot(df_mat['Voltage_kV'], df_mat['Phase_max_rad'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ax3.axhline(np.pi/2, color='red', linestyle='--', linewidth=2, label='WPOA limit (π/2)')
    ...
    ax4.bar(x + width/2, df['Contrast_Classical'], width, label='Classical', alpha=0.8)
    ...
    ax4.set_xticks(x)
    ax4.set_xticklabels([f"{row['Material']}\n{row['Voltage_kV']:.0f}kV" 
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax5.plot(df_mat['Voltage_kV'], df_mat['Fidelity'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ax5.axhline(0.99, color='green', linestyle='--', linewidth=1, alpha=0.5, label='Target (0.99)')
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax6.plot(df_mat['Voltage_kV'], df_mat['SSIM'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ax6.axhline(0.95, color='green', linestyle='--', linewidth=1, alpha=0.5, label='Target (0.95)')
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax7.plot(df_mat['Voltage_kV'], df_mat['Pearson'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ax7.axhline(0.99, color='green', linestyle='--', linewidth=1, alpha=0.5, label='Target (0.99)')
    ...
    for mat, color in zip(materials, colors):
        df_mat = df[df['Material'] == mat]
        ax8.plot(df_mat['Voltage_kV'], df_mat['RMSE'], 'o-', 
                label=mat, color=color, linewidth=2, markersize=8)
    ax8.axhline(0.01, color='green', linestyle='--', linewidth=1, alpha=0.5, label='Target (0.01)')
    ...
    summary_text += f"  Fidelity:  {df['Fidelity'].mean():.4f} ± {df['Fidelity'].std():.4f}\n"
    summary_text += f"  SSIM:      {df['SSIM'].mean():.4f} ± {df['SSIM'].std():.4f}\n"
    summary_text += f"  Pearson:   {df['Pearson'].mean():.4f} ± {df['Pearson'].std():.4f}\n"
    ...
    fig.suptitle('Comprehensive Quantum TEM Validation Suite', 

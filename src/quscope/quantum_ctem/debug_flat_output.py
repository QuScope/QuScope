...
print(f"\nTransmission function:")
print(f"  |t| range: [{np.abs(transmission).min():.6f}, {np.abs(transmission).max():.6f}]")
print(f"  Phase range: [{np.angle(transmission).min():.6f}, {np.angle(transmission).max():.6f}] rad")
...
print(f"  |ψ| range: [{np.abs(psi_incident).min():.6e}, {np.abs(psi_incident).max():.6e}]")
...
print(f"  Contrast: {(intensity_direct.max()-intensity_direct.min())/intensity_direct.mean():.6f}")
...
print(f"  Contrast: {(intensity_sim.max()-intensity_sim.min())/intensity_sim.mean():.6f}")

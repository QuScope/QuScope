...
def test_sign_combinations(voltage_kv=200.0, defocus_A=200.0):
    ...
    atoms = mx2(
        formula="MoS2", kind="2H", a=3.18, thickness=3.19, size=(3, 3, 1), vacuum=7.5
    )
    ...
    try:
        ...
        pot_classical = abtem.Potential(
            atoms, gpts=256, sampling=0.1, device="cpu", projection="infinite"
        ).build()

        probe_classical = abtem.PlaneWave(energy=voltage_kv * 1000, device="cpu")
        ...
        print(
            f"  Range: [{intensity_classical.min():.6e}, {intensity_classical.max():.6e}]"
        )
    ...
    print(
        f"{'Combo':<8} {'Phase':<7} {'CTF':<7} {'Fresnel':<9} {'Pearson':<10} {'SSIM':<10}"
    )
    print(f"{'-'*70}")
    ...
    for idx, (sign_phase, sign_ctf, sign_fresnel) in enumerate(sign_combinations):
        ...
        V = sim.potential_converter.atoms_to_potential(
            atoms, params.grid_size, params.pixel_size
        )
        ...
        from quscope.quantum_ctem.hamiltonian import (
        ...
        if intensity_classical is not None:
            ...
            results.append(
                {
                    ...
                    "ssim": ssim,
                    "intensity_range": (
                        intensity_quantum.min(),
                        intensity_quantum.max(),
                    ),
                }
            )
            ...
            print(
                f"{status} {idx:<6} {sign_phase:<7} {sign_ctf:<7} {sign_fresnel:<9} "
                f"{pearson:>9.4f} {ssim:>9.4f}"
            )
        else:
            print(
                f"  {idx:<6} {sign_phase:<7} {sign_ctf:<7} {sign_fresnel:<9} "
...
def visualize_sign_test(voltage_kv=200.0, defocus_A=200.0):
    ...
    ax.text(
        0.5,
        0.6,
        f"Phase grating: exp({best['sign_phase']}iσV)",
        ha="center",
        fontsize=11,
        family="monospace",
    )
    ...
    ax.text(
        0.5,
        0.3,
        f"Fresnel: exp({best['sign_fresnel']}iπλzk²)",
        ha="center",
        fontsize=11,
        family="monospace",
    )
    ...
    ax.text(
        0.5,
        0.6,
        f"Phase grating: exp(+iσV)",
        ha="center",
        fontsize=11,
        family="monospace",
    )
    ax.text(0.5, 0.45, f"CTF: exp(+iχ)", ha="center", fontsize=11, family="monospace")
    ax.text(
        0.5, 0.3, f"Fresnel: exp(+iπλzk²)", ha="center", fontsize=11, family="monospace"
    )
    ax.text(
        0.5,
        0.1,
        f"Pearson: {current['pearson']:.4f}",
        ha="center",
        fontsize=11,
        fontweight="bold",
        color=color,
    ...
    # Save figure
    output_path = (
        Path(__file__).parent.parent.parent.parent / "sign_inversion_analysis.png"
    )

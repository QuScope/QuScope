...
def diagnose_potential_scaling():
    ...
    stats = {
        "phase_shift_max": np.abs(phase_quantum).max(),
    ...
    ax2.set_title(
        f"Classical Potential (abTEM)\nMax: {V_classical.max():.2e} V·Å",
        fontweight="bold",
    )
    ax2.axis("off")
...
"""
    ...
    plt.suptitle(
        "MoS₂ Potential Scaling Diagnostic (200 kV)", fontsize=16, fontweight="bold"
    )
    plt.tight_layout()

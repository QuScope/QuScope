...
def main():
    ...
    pearson_norm = metrics.calculate_pearson_correlation(I_quantum_norm, I_classical_norm)
    ...
    ax3.set_title(f'Absolute Difference (Raw)\nMean: {diff_raw.mean():.2e}',
                  fontsize=11, fontweight='bold')
    ax3.axis('off')
    ...
    ax4.set_title(f'Potential V(x,y)\nMax: {V.max():.1f} V·Å',
                  fontsize=11, fontweight='bold')
    ax4.axis('off')
    ...
    ax5.set_title('Classical (Normalized)\nRange: [0.0, 1.0]',
                  fontsize=11, fontweight='bold')
    ax5.axis('off')
    ...
    ax6.set_title('Quantum (Normalized)\nRange: [0.0, 1.0]',
                  fontsize=11, fontweight='bold')
    ax6.axis('off')
    ...
    ax7.set_title(f'Absolute Difference (Normalized)\nRMSE: {rmse_norm:.4f}',
                  fontsize=11, fontweight='bold')
    ax7.axis('off')
    ...
    ax8.scatter(I_classical_norm.flatten()[::10], I_quantum_norm.flatten()[::10],
    ...
    ax8.set_title(f'Pixel Correlation\nPearson: {pearson_norm:.4f}',
                  fontsize=11, fontweight='bold')
    ...
    ax9.plot(I_classical_norm[center, :], 'b-', linewidth=2, label='Classical', alpha=0.7)
    ...
    ax9.set_title('Line Profile (Y = center, Normalized)', fontsize=11, fontweight='bold')
    ax9.grid(True, alpha=0.3)
    ax9.legend()
    
    # Intensity histograms
    ax10 = fig.add_subplot(gs[2, 2:])
    ax10.hist(I_classical_norm.flatten(), bins=100, alpha=0.5, label='Classical', color='blue', density=True)
    ax10.hist(I_quantum_norm.flatten(), bins=100, alpha=0.5, label='Quantum', color='red', density=True)
    ...
    # Save
    plt.savefig('quantum_classical_comparison_normalized.png', dpi=300, bbox_inches='tight')

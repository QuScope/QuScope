...
try:
    ...
    raise ImportError("abTEM is required. Install with: conda install -c conda-forge abtem")
...
def visualize_mos2_structure(atoms: Atoms, save_path: str = None):
    ...
    if mo_positions:
        ...
        ax1.scatter(mo_pos[:, 0], mo_pos[:, 1], mo_pos[:, 2], 
                   c='purple', s=200, label='Mo', alpha=0.8)
    
    if s_positions:
        s_pos = np.array(s_positions)
        ax1.scatter(s_pos[:, 0], s_pos[:, 1], s_pos[:, 2], 
                   c='yellow', s=100, label='S', alpha=0.8)
    ...
    if mo_positions:
        ax2.scatter(mo_pos[:, 0], mo_pos[:, 1], c='purple', s=200, 
                   label='Mo', alpha=0.8, edgecolors='black', linewidths=1)
    if s_positions:
        ax2.scatter(s_pos[:, 0], s_pos[:, 1], c='yellow', s=100, 
                   label='S', alpha=0.8, edgecolors='black', linewidths=1)
    ax2.set_xlabel('x (Å)')
    ax2.set_ylabel('y (Å)')
    ...
    if mo_positions:
        ax3.scatter(mo_pos[:, 0], mo_pos[:, 2], c='purple', s=200, 
                   label='Mo', alpha=0.8, edgecolors='black', linewidths=1)
    if s_positions:
        ax3.scatter(s_pos[:, 0], s_pos[:, 2], c='yellow', s=100, 
                   label='S', alpha=0.8, edgecolors='black', linewidths=1)
    ax3.set_xlabel('x (Å)')
    ax3.set_ylabel('z (Å)')
    ...
    plt.suptitle(f'MoS₂ Atomic Structure ({len(atoms)} atoms)', 
...
def compare_potentials_mos2(
    ...
    # Correlation plot
    axes[1, 1].hexbin(V_quantum.flatten(), V_classical.flatten(), 
                      gridsize=50, cmap='viridis', mincnt=1)
    axes[1, 1].plot([V_quantum.min(), V_quantum.max()], 
...
def run_mos2_validation(
    ...
    if verbose:
        print(f"    ✓ Classical intensity: [{float(classical_image.min()):.6f}, {float(classical_image.max()):.6f}]")
    ...
    if verbose:
        print(f"    ✓ Quantum intensity: [{float(quantum_image.min()):.6f}, {float(quantum_image.max()):.6f}]")
...
def run_multi_voltage_mos2_validation(
    voltages: list = [80e3, 120e3, 200e3, 300e3],
    grid_size: int = 256
) -> Dict[float, Dict]:
    ...
    for voltage in voltages:
        ...
        wavelength = h / np.sqrt(2 * m_e * e * voltage * (1 + e * voltage / (2 * m_e * c**2)))
    ...
    ValidationVisualizer.plot_multi_voltage_validation(
        results,
        save_path='validation_mos2_multi_voltage.png'
    )

...
class QuantumWaveEncoder:
    ...
    def __init__(self, grid_size: int):
        ...
        print(f"  Quantum state space: 2^{self.n_qubits_total} = {2**self.n_qubits_total}")
    ...
    def encode_amplitude(self, psi_array: np.ndarray) -> QuantumCircuit:
        ...
        """
        # Verify size
        assert psi_array.shape == (self.N, self.N), \
    ...
    def resource_estimate(self) -> dict:
        ...
        amp_encoding_depth = O(2**self.n_qubits_total)  # Exponential!
        amp_encoding_gates = O(2**(2*self.n_qubits_total))  # Very expensive
...
class QuantumPhaseGrating:
    ...
    def apply(
        ...
        print(f"  Phase grating: {len(phase_groups)} unique phases for {self.N}×{self.N} pixels")
...
def demo_quantum_encoding():
    ...
    print(f"  Plane wave encoding: {resources['plane_wave']['gates']} gates, depth {resources['plane_wave']['depth']}")
    ...
    for size in [8, 16, 32, 64, 128, 256]:
        ...
        print(f"  {size:3d}×{size:<3d}  |   {enc.n_qubits_total:2d}   |        {enc.n_qubits_total:3d}         | 2^{enc.n_qubits_total} = {2**enc.n_qubits_total}")
...
Feasibility: {"YES - within NISQ limits!" if N <= 32 else "NO - needs fault-tolerant QC"}
    """
    axes[1, 1].text(0.1, 0.9, circuit_text, transform=axes[1, 1].transAxes,

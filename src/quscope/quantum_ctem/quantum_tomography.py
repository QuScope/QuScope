...
try:
    from quscope.quantum_ctem.quantum_simulation import QuantumTEMSimulator, QuantumSimulationParameters
except ImportError:
...
class QuantumTomography:
    ...
    def run_defocus_series(
        ...
        probe_ref = abtem.PlaneWave(
            energy=self.voltage_kv * 1000,
            device='cpu'
        )
        ...
        if verbose:
            ...
            contrast_exit = (intensity_exit.max() - intensity_exit.min()) / intensity_exit.max()
            print(f"  Exit wave contrast: {contrast_exit:.3f}")
            print(f"  Exit wave intensity range: [{intensity_exit.min():.3e}, {intensity_exit.max():.3e}]\n")
        
        # Now apply different defocus values to this exit wave
        for i, defocus in enumerate(defocus_values):
            if verbose:
                print(f"[{i+1}/{len(defocus_values)}] Defocus = {defocus:>6.0f} Å...", end=" ")
            ...
            if exit_wave.shape != (grid_size, grid_size):
                ...
                if exit_wave.shape != (grid_size, grid_size):
                    print(f"    Warning: unexpected shape {exit_wave.shape}, using last quantum wave or reference exit wave")
    ...
    def _extract_quantum_phase(self, intensity: np.ndarray, defocus: float) -> np.ndarray:
        """
    ...
    def _calculate_ctf(self, grid_size: int, pixel_size: float, defocus: float) -> np.ndarray:
        """
    ...
    def _perform_tomography(self, atoms, verbose: bool = True) -> Dict:
        ...
        if verbose:
            ...
            print(f"  Retrievable: {'✓ YES' if phase_quality['retrievable'] else '✗ NO'}")
    ...
    def visualize_tomography(self, results: Dict, output_path: str = None):
        ...
        for i in range(min(4, n_defocus)):
            ...
            ax.set_title(f'Defocus = {self.defocus_series[i]:.0f} Å\nIntensity |ψ|²', 
                        fontsize=10)
        ...
        for i in range(min(4, n_defocus)):
            ...
            im = ax.imshow(phase, cmap='twilight', vmin=-np.pi, vmax=np.pi, origin='lower')
            ax.set_title(f'Defocus = {self.defocus_series[i]:.0f} Å\nPhase φ(x,y)', 
                        fontsize=10)
            ax.set_xlabel('x (pixels)')
            ax.set_ylabel('y (pixels)')
        ...
        bars = ax1.bar(info_types, bits, color=colors, alpha=0.7, edgecolor='black', linewidth=2)
        ax1.set_ylabel('Information Content (bits)', fontsize=12, fontweight='bold')
        ax1.set_title('(a) Information Content: Classical vs Quantum', 
        ...
        ax1.text(0.5, 0.95, f'Quantum Advantage: {advantage:.1f}× more information',
        ...
        ax2.plot(defocus_vals, phase_ranges, 'o-', linewidth=2, markersize=8, 
                color='purple', label='Phase range')
        ax2.axhline(np.pi/4, color='red', linestyle='--', linewidth=2, 
                   label='Retrievability threshold (π/4)')
        ax2.axhline(np.pi/2, color='orange', linestyle='--', linewidth=2, 
        ...
        ax3.set_title(f'(c) Amplitude |ψ|\n(Defocus={defocus_mid:.0f} Å)', fontsize=11, fontweight='bold')
        ...
        phase = np.angle(wave)
        im4 = ax4.imshow(phase, cmap='twilight', vmin=-np.pi, vmax=np.pi, origin='lower')
        ax4.set_title(f'(d) Phase φ\n(Defocus={defocus_mid:.0f} Å)', fontsize=11, fontweight='bold')
        ...
        ax6.set_title(f'(f) Imag[ψ]\n(Defocus={defocus_mid:.0f} Å)', fontsize=11, fontweight='bold')
        ax6.set_xlabel('x (pixels)')
        ax6.set_ylabel('y (pixels)')
        plt.colorbar(im6, ax=ax6, fraction=0.046)
        
        # Main title
        fig.suptitle(f'Quantum Tomography for Electron Microscopy\n' +
        ...
        if output_path is None:
            output_path = Path(__file__).parent.parent.parent.parent / 'quantum_tomography.png'

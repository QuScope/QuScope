...
def analyze_validity_regime(df):
    ...
    for mat in df['Material'].unique():
        ...
        if in_regime_mat.any():
            print(f"    |Pearson| (in regime): {pearson_mat[in_regime_mat].mean():.3f} ⭐")
        if (~in_regime_mat).any():
            print(f"    |Pearson| (out regime): {pearson_mat[~in_regime_mat].mean():.3f}")
    
    return phase_max, pearson, pearson_abs, phase_theory, pearson_theory, phase_threshold


def create_validity_figure(phase_max, pearson, pearson_abs, phase_theory, 
                           pearson_theory, phase_threshold, df):
    """
    ...
    ax1.plot(phase_theory/np.pi, pearson_theory, 'k--', linewidth=2.5, 
             label='Theoretical: $r = \\exp(-\\phi^2/2)$', zorder=1)
    
    # Plot validity boundary
    ax1.axvline(phase_threshold/np.pi, color='red', linestyle='--', linewidth=2,
                alpha=0.7, label=f'WPOA boundary ($\\phi$ = {phase_threshold/np.pi:.2f}π)')
    ax1.axhspan(0.5, 1.0, alpha=0.1, color='green', label='Good agreement (r > 0.5)')
    
    # Plot data points
    for mat in materials:
        mask = df['Material'] == mat
        ax1.scatter(phase_max[mask]/np.pi, pearson_abs[mask], 
                   s=100, alpha=0.7, color=colors.get(mat, 'gray'), 
                   marker=markers.get(mat, 'o'), label=mat, zorder=2, edgecolors='black')
    ...
    ax1.set_title('(a) WPOA Validity Regime: Theory vs Experiment', 
    ...
    ax2.bar(x - width/2, mat_pearson_in, width, label='In WPOA regime', 
            color='green', alpha=0.7, edgecolor='black')
    ax2.bar(x + width/2, mat_pearson_out, width, label='Out of WPOA regime', 
    ...
    for mat in materials:
        ...
        ax3.plot(voltages, pearson_mat, marker=markers.get(mat, 'o'), 
                linewidth=2, markersize=8, label=mat, 
                color=colors.get(mat, 'gray'), alpha=0.7)
    ...
    for mat in materials:
        mask = df['Material'] == mat
        ax4.scatter(phase_max[mask]/np.pi, ssim[mask], 
                   s=100, alpha=0.7, color=colors.get(mat, 'gray'), 
                   marker=markers.get(mat, 'o'), label=mat, edgecolors='black')
    
    ax4.axvline(phase_threshold/np.pi, color='red', linestyle='--', linewidth=2, alpha=0.7)
...
"""
    ...
    # Main title
    fig.suptitle('Quantum Weak Phase Approximation: Validity Regime Analysis\n' +
                 '16 validation cases across 4 materials and 4 voltages',
                 fontsize=14, fontweight='bold')
    
    # Save
    output_path = Path(__file__).parent.parent.parent.parent / 'wpoa_validity_analysis.png'
...
def main():
    ...
    if df is None:
        ...
        print("  Please run: python src/quscope/quantum_ctem/comprehensive_validation.py")
        return
    ...
    # Create figure
    create_validity_figure(phase_max, pearson, pearson_abs, phase_theory, 
                          pearson_theory, phase_threshold, df)

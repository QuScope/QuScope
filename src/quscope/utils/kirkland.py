...
class KirklandPotential:
    ...
    def load_parameters(self):
        try:
            ...
            print(
                f"Warning: Kirkland parameter file '{self.params_file}' not found. Using empty parameter set."
            )
            return None
        except json.JSONDecodeError as e:
            print(
                f"Warning: Failed to parse Kirkland parameter file '{self.params_file}': {e}"
            )
            return None
    ...
    def kirkland_potential_2d(self, x_grid, y_grid, atom_x, atom_y, Z, element=None):
        ...
        if element not in self.parameters:
            raise ValueError(
                f"Element with Z={Z} ({element}) not found in Kirkland parameters"
            )

...
class TestRoundTripAccuracy:
    ...
    def test_random_wave_round_trip(self):
        ...
        # Random complex wave
        np.random.seed(42)
        psi_original = (np.random.randn(4, 4) + 
                       1j * np.random.randn(4, 4))
...
class TestEdgeCases:
    ...
    def test_maximum_amplitude_values(self):
        ...
        assert np.isclose(np.linalg.norm(psi_large), 
                         np.linalg.norm(psi_extracted), 
                         rtol=1e-10)
    ...
    def test_very_small_amplitude_values(self):
        ...
        assert np.isclose(np.linalg.norm(psi_small), 
                         np.linalg.norm(psi_extracted), 
                         rtol=1e-6)

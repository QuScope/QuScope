...
class TestIBMDeviceProfile:
    ...
    def test_ibm_brisbane_profile(self):
        ...
        assert profile.single_qubit_error <= IBMDeviceProfile.ibm_kyoto().single_qubit_error
...
class TestIBMHardwareValidator:
    ...
    def test_compare_devices(self):
        ...
        assert all(device in comparison for device in 
                  ['ibm_kyoto', 'ibm_brisbane', 'ibm_nazca', 'ibm_sherbrooke'])
        
        # All should have fidelity estimates
        assert all('estimated_fidelity' in results 
...
class TestValidationWarnings:
    ...
    def test_low_fidelity_warning(self):
        ...
        assert any('fidelity' in warning.lower() 
    ...
    def test_depth_increase_warning(self):
        ...
        if results['transpiled_depth'] > 2 * results['original_depth']:
            assert any('depth' in warning.lower() 
                      for warning in results['warnings'])

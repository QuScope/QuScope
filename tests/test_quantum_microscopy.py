...
class TestQuantumMicroscopy:
    ...
    @pytest.mark.skipif(
        not ENCODING_AVAILABLE,
        reason=f"Encoding not available: {ENCODING_ERROR if not ENCODING_AVAILABLE else ''}",
    )
    ...
    @pytest.mark.skipif(
        ...
        reason=f"Backend not available: {BACKEND_ERROR if not BACKEND_AVAILABLE else ''}",
    )
    ...
    @pytest.mark.skipif(
        not QUSCOPE_AVAILABLE,
        reason=f"QuScope not available: {QUSCOPE_ERROR if not QUSCOPE_AVAILABLE else ''}",
    )
